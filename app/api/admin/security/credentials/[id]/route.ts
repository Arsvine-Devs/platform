...
import { isDevelopmentBypassEnabled, isDevelopmentBypassSession, revokeDevelopmentCredential } from '@/lib/development-preview';
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  if (isDevelopmentBypassEnabled()) {
    ...
    if (developmentSession && isDevelopmentBypassSession(developmentSession)) {
      ...
      if (!revokeDevelopmentCredential(id)) return privateJson({ ok: false, error: { message: 'At least one security key must remain.' } }, { status: 422 });
  ...
  const limiter = await enforceRateLimit(`webauthn-credentials-revoke:${getClientKey(request)}`, 12, 10 * 60_000);
  if (!limiter.ok) {
  ...
  if (!isRecentWebAuthnSession(session)) return privateJson({ ok: false, error: { code: 'reauth_required', message: '请重新使用安全密钥登录后再撤销密钥。' } }, { status: 401 });
  ...
  try {
    ...
    return privateJson({ ok: false, error: { message: error instanceof Error ? error.message : '无法撤销安全密钥。' } }, { status: 422 });
  }
}
