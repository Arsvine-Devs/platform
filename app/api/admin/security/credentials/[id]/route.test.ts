...
function request() {
  return new NextRequest('https://ctrl.arsvine.com/api/admin/security/credentials/credential-record', {
    method: 'DELETE',
...
describe('DELETE /api/admin/security/credentials/[id]', () => {
  it('requires recent WebAuthn and invalidates sessions after revocation', async () => {
    const response = await DELETE(request(), { params: Promise.resolve({ id: 'credential-record' }) });
