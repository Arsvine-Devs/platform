...
export async function POST(request: NextRequest) {
      ...
      return privateJson(
        {
          ok: false,
          error: {
            code: 'development_bypass',
            message: 'Use the local preview security-key flow.',
          },
        },
        { status: 409 },
      );
  ...
  const limiter = await enforceRateLimit(
    `webauthn-registration-options:${getClientKey(request)}`,
    12,
    10 * 60_000,
  );
  if (!limiter.ok) {
  ...
  try {
    ...
    const managedReauthentication =
      owner.authMethod === 'webauthn' && isRecentWebAuthnSession(session);
    if (!firstMigration && !managedReauthentication) {
      return privateJson(
        {
          ok: false,
          error: { code: 'reauth_required', message: '请重新使用安全密钥登录后再管理密钥。' },
        },
        { status: 401 },
      );
    }
    ...
    const options = await createRegistrationOptions(
      ...
      credentials.map(toStoredWebAuthnCredential),
    );
    ...
    return privateJson(
      { ok: false, error: { message: '无法开始安全密钥注册。' } },
      { status: 422 },
    );
  }
}
