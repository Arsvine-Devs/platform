...
import { applyWebAuthnCeremonyCookie, createAuthenticationOptions, getWebAuthnConfig } from '@/lib/webauthn';
import { createWebAuthnChallenge, getOwnerAccount, listActiveWebAuthnCredentials } from '@/lib/webauthn-store';
import { POST } from './route';

const owner = { id: '00000000-0000-4000-8000-000000000001', role: 'owner' as const, status: 'active' as const, authMethod: 'webauthn' as const };
...
beforeEach(() => {
  ...
  vi.mocked(listActiveWebAuthnCredentials).mockResolvedValue([{ id: 'record', credentialId: 'credential', publicKey: 'public', counter: 0, label: 'key', aaguid: 'aaguid', attestationFormat: 'packed', transports: '["usb"]', deviceType: 'singleDevice', backedUp: false, createdAt: new Date(), lastUsedAt: null, revokedAt: null }] as never);
  vi.mocked(getWebAuthnConfig).mockReturnValue({ rpId: 'ctrl.arsvine.com', origin: 'https://ctrl.arsvine.com', rpName: 'ARSVINE Admin' });
  vi.mocked(createAuthenticationOptions).mockResolvedValue({ challenge: 'challenge', rpId: 'ctrl.arsvine.com', allowCredentials: [], userVerification: 'required' });
...
function request() {
  return new NextRequest('https://ctrl.arsvine.com/api/admin/webauthn/authentication/options', { method: 'POST' });
}
...
describe('POST /api/admin/webauthn/authentication/options', () => {
  it('returns options and binds a short-lived ceremony cookie', async () => {
    ...
    expect(createWebAuthnChallenge).toHaveBeenCalledWith(expect.objectContaining({ type: 'authentication', challenge: 'challenge', userId: owner.id }));
  ...
  it('does not expose authentication options before Owner migration', async () => {
    vi.mocked(getOwnerAccount).mockResolvedValue({ ...owner, authMethod: 'password+totp' } as never);
    const response = await POST(request());
