...
import { getWebAuthnCeremonyId, isAuthenticationResponse, isHardwareOrientedCredential, verifyAuthentication } from '@/lib/webauthn';
...
const storedCredential = { id: 'credential-record', credentialId: 'credential-id', publicKey: 'public-key', counter: 2, transports: '["usb"]' };
const authenticationResponse = { id: 'credential-id', rawId: 'credential-id', type: 'public-key', response: { clientDataJSON: 'client-data', authenticatorData: 'authenticator-data', signature: 'signature' } };
...
function request(body: unknown, ceremonyId = '00000000-0000-4000-8000-000000000010') {
  return new NextRequest('https://ctrl.arsvine.com/api/admin/webauthn/authentication/verify', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', cookie: `arsvine_webauthn_ceremony=${ceremonyId}` },
...
beforeEach(() => {
  ...
  vi.mocked(consumeWebAuthnChallenge).mockResolvedValue({ id: '00000000-0000-4000-8000-000000000010', userId: owner.id, type: 'authentication', challenge: 'challenge', rpId: 'ctrl.arsvine.com', origin: 'https://ctrl.arsvine.com', label: null, sessionBindingHash: null, expiresAt: new Date(Date.now() + 60_000), consumedAt: new Date(), createdAt: new Date() } as never);
  vi.mocked(getActiveWebAuthnCredential).mockResolvedValue(storedCredential as never);
  vi.mocked(toWebAuthnVerificationCredential).mockReturnValue({ id: 'credential-id', publicKey: new Uint8Array([1]), counter: 2, transports: ['usb'] });
  vi.mocked(verifyAuthentication).mockResolvedValue({ verified: true, authenticationInfo: { credentialID: 'credential-id', newCounter: 3, userVerified: true, credentialDeviceType: 'singleDevice', credentialBackedUp: false, origin: 'https://ctrl.arsvine.com', rpID: 'ctrl.arsvine.com' } });
  vi.mocked(isHardwareOrientedCredential).mockReturnValue(true);
  vi.mocked(updateWebAuthnCredentialAfterAuthentication).mockResolvedValue(storedCredential as never);
...
describe('POST /api/admin/webauthn/authentication/verify', () => {
  it('creates a WebAuthn session only after challenge and assertion verification', async () => {
    const response = await POST(request({ ceremonyId: '00000000-0000-4000-8000-000000000010', response: authenticationResponse }));
    ...
    expect(consumeWebAuthnChallenge).toHaveBeenCalledWith({ id: '00000000-0000-4000-8000-000000000010', userId: owner.id, type: 'authentication' });
    expect(updateWebAuthnCredentialAfterAuthentication).toHaveBeenCalledWith(expect.objectContaining({ counter: 3, credentialId: 'credential-id' }));
