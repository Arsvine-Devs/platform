...
beforeEach(() => {
  ...
  vi.mocked(enforceRateLimit).mockResolvedValue({ ok: true, remaining: 4, retryAfterMs: 60_000 });
  vi.mocked(ensureOwnerBootstrap).mockResolvedValue(account as never);
  vi.mocked(getAccountByEmail).mockResolvedValue(account as never);
  vi.mocked(verifyPasswordHash).mockReturnValue(true);
  vi.mocked(decryptSecret).mockReturnValue(JSON.stringify({ current: 'JBSWY3DPEHPK3PXP' }));
...
describe('POST /api/admin/login', () => {
  ...
  it('returns a generic failure for invalid credentials', async () => {
    ...
    const response = await POST(
      request({ email: 'owner@example.com', password: 'wrong', totpToken: '123456' }),
    );
    expect(response.status).toBe(401);
    expect((await response.json()).error.message).toBe('登录失败，请检查凭据。');
  });
  it('creates signed auth and csrf cookies only after password and TOTP pass', async () => {
    const response = await POST(
      request({ email: 'owner@example.com', password: 'correct', totpToken: '123456' }),
    );
  ...
  it('accepts a configured previous TOTP secret during rotation', async () => {
    vi.mocked(decryptSecret).mockReturnValue(
      JSON.stringify({ current: 'CURRENT', previous: ['PREVIOUS'] }),
    );
  ...
  it('keeps password plus TOTP login for Editors', async () => {
    ...
    const response = await POST(
      request({ email: editor.email, password: 'correct', totpToken: '123456' }),
    );
    expect(response.status).toBe(200);
    expect((await response.json()).data).toMatchObject({
      role: 'editor',
      authMethod: 'password+totp',
      needsWebAuthnSetup: false,
    });
  });
});
