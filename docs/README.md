# ARSVINE PLATFORM 文档

[返回项目主页](../README.md) · [返回仓库地图](../INDEX.md) · [文档目录](./INDEX.md)

本文档组面向维护者和运维人员，说明 Platform 当前的服务边界、运行方式、配置与安全责任。事实以当前源码、manifest、Schema、migration 和可执行检查为准。

## 从这里开始

- 第一次进入仓库：[`DEVELOPMENT.md`](./DEVELOPMENT.md)
- 判断改动属于哪个服务：[`ARCHITECTURE.md`](./ARCHITECTURE.md)
- 配置健康检查、发布链路和故障边界：[`OPERATIONS.md`](./OPERATIONS.md)
- 修改身份、Token、Session 或 storage：[`SECURITY.md`](./SECURITY.md)
- 具体服务和包：从 [`INDEX.md`](./INDEX.md) 进入对应入口

人类文档使用简体中文；命令、路径、环境变量、API、协议字段和错误码保持权威形式。AI 操作文档 `AGENTS.md` 使用技术英语。
