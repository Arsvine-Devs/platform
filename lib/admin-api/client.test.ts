...
describe('adminRequest', () => {
  it('adds JSON, no-store, and CSRF request metadata', async () => {
    ...
    await expect(adminRequest<{ saved: boolean }>('/api/admin/example', {
      method: 'PUT',
  ...
  it('supports successful responses without a data property', async () => {
    ...
    await expect(adminRequest<void>('/api/admin/example', { method: 'DELETE' })).resolves.toBeUndefined();
  });
  ...
  it('preserves server error code and retry metadata', async () => {
    ...
    await expect(result).rejects.toMatchObject({ status: 401, code: 'reauth_required', retryAfterMs: 3000, message: '请重新认证。' });
  });
  ...
  it('returns a safe error for non-JSON responses and network failures', async () => {
    ...
    await expect(adminRequest('/api/admin/example')).rejects.toMatchObject({ status: 502, message: '请求失败（HTTP 502）。' });

    fetchMock.mockRejectedValue(new Error('connection reset'));
    await expect(adminRequest('/api/admin/example')).rejects.toMatchObject({ status: 0, message: '网络请求失败，请检查连接后重试。' });
  });
});
