...
describe('signed sessions', () => {
  ...
  it('rejects a tampered session before querying the account', async () => {
    ...
    const raw = JSON.parse(Buffer.from(session.value, 'base64url').toString('utf8')) as Record<
      string,
      unknown
    >;
    raw.role = 'editor';
    const request = new NextRequest('http://localhost/library', {
      headers: {
        cookie: `arsvine_admin_session=${Buffer.from(JSON.stringify(raw)).toString('base64url')}`,
      },
    });
  ...
  it('resolves the signed local preview session without querying the account store', async () => {
    ...
    const request = new NextRequest('http://localhost/library', {
      headers: { cookie: `${DEVELOPMENT_SESSION_COOKIE}=${session.value}` },
    });
  ...
  it('rejects a tampered local preview identity', async () => {
    ...
    const request = new NextRequest('http://localhost/library', {
      headers: {
        cookie: `${DEVELOPMENT_SESSION_COOKIE}=${Buffer.from(JSON.stringify(raw)).toString('base64url')}`,
      },
    });
