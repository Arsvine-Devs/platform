...
export default function TweetListPanel({
  ...
  return (
    <section
      ...
      aria-label={t('tweets.list')}
    >
      <div className="flex flex-col gap-4 border-b pb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="font-heading text-lg font-semibold">{monthLabel}</h2>
          <p className="mt-1 text-xs text-muted-foreground">{t('tweets.listHint')}</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Tabs
            value={filter}
            onValueChange={(value) => onFilterChange((value ?? 'all') as TweetFilter)}
          >
            <TabsList>
              {TWEET_FILTERS.map((item) => (
          ...
          <Button type="button" className="min-h-10" onClick={onCreate}>
      ...
      <div className="pt-4">
        {loading ? (
          <LoadingState label={t('tweets.loading')} />
        ) : tweets.length === 0 ? (
          <EmptyState
            title={t('tweets.noMatch')}
            description={emptyHint}
            action={
              <Button type="button" variant="outline" className="min-h-10" onClick={onCreate}>
                {t('tweets.writeOne')}
              </Button>
            }
            className="min-h-32"
