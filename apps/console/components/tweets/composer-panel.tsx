...
export default function ComposerPanel({
  ...
  return (
    <form
      ...
      aria-label={
        mode === 'edit' ? t('tweets.editTitle', { id: editingTweet?.id ?? '' }) : t('tweets.new')
      }
    >
      <div className="flex items-start justify-between gap-3 border-b pb-4">
        <div>
          <p className="text-xs font-medium uppercase tracking-[0.14em] text-muted-foreground">
            {mode === 'edit' ? t('tweets.edit') : t('tweets.compose')}
          </p>
          <h2 className="mt-1 font-heading text-lg font-semibold">
            {mode === 'edit' && editingTweet
              ? t('tweets.editTitle', { id: editingTweet.id })
              : t('tweets.composeTitle')}
          </h2>
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            {isExternalSource
              ? t('tweets.externalHint')
              : mode === 'edit'
                ? t('tweets.localEditHint')
                : t('tweets.newHint')}
          </p>
        </div>
        <Button
      ...
      <FieldGroup>
        <Field>
          <FieldLabel htmlFor="tweet-content">{t('tweets.body')}</FieldLabel>
          <Textarea
            id="tweet-content"
            value={form.content}
            disabled={isExternalSource}
            onChange={(event) => onChange('content', event.target.value)}
            placeholder={t('tweets.bodyPlaceholder')}
            rows={7}
          />
          <FieldDescription>{t('tweets.bodyHint')}</FieldDescription>
        </Field>
        <div className="grid gap-4 sm:grid-cols-2">
          <Field>
            <FieldLabel htmlFor="tweet-created-at">{t('tweets.publishTime')}</FieldLabel>
            <Input
              id="tweet-created-at"
              type="datetime-local"
          ...
          <Field>
            <FieldLabel htmlFor="tweet-lang">{t('common.language')}</FieldLabel>
            <Select
              disabled={isExternalSource}
              value={form.lang}
              onValueChange={(value) => onChange('lang', value ?? 'zh-CN')}
            >
              <SelectTrigger id="tweet-lang">
                <SelectValue>{(value) => tweetLanguageLabel(value ?? 'zh-CN', t)}</SelectValue>
              </SelectTrigger>
              <SelectContent>
                {TWEET_LANGS.map((lang) => (
                  <SelectItem key={lang} value={lang}>
                    {tweetLanguageLabel(lang, t)}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <Field>
            <FieldLabel htmlFor="tweet-visibility">{t('tweets.visibility')}</FieldLabel>
            <Select
              value={form.visibility}
              onValueChange={(value) =>
                onChange('visibility', (value ?? 'public') as TweetVisibility)
              }
            >
              <SelectTrigger id="tweet-visibility">
                <SelectValue>
                  {(value) => visibilityLabel((value ?? 'public') as TweetVisibility, t)}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
                {TWEET_VISIBILITIES.map((value) => (
          ...
          <Field>
            <FieldLabel htmlFor="tweet-tags">{t('tweets.tags')}</FieldLabel>
            <Input
              id="tweet-tags"
              value={form.tags}
              onChange={(event) => onChange('tags', event.target.value)}
              placeholder={t('tweets.tagsPlaceholder')}
            />
          </Field>
        </div>
        <div className="grid gap-2">
          <div className="flex min-h-11 items-center justify-between gap-3 rounded-xl border px-3">
            <label htmlFor="tweet-pinned" className="text-sm">
              {t('tweets.pinned')}
            </label>
            <Switch
              id="tweet-pinned"
          ...
          <div className="rounded-xl border bg-muted/20 p-4">
            <div className="flex flex-wrap gap-1.5">
              {summary.map((item) => (
                ...
                >
                  {translationSummaryLabel(item.locale, item.state, t)}
                </Badge>
              ))}
            </div>
      ...
      <div className="flex flex-col gap-3 border-t pt-4 sm:flex-row sm:items-center">
        ...
        >
          {saving ? (
            ...
            <>
              <Save />
              {mode === 'edit' ? t('tweets.save') : t('tweets.create')}
            </>
          )}
        </Button>
        <Button
...
function translationSummaryLabel(
  locale: string,
...
function tweetLanguageLabel(value: string, t: Translate) {
  const keys: Record<string, string> = {
    'zh-CN': 'tweets.lang.zhCN',
    'zh-TW': 'tweets.lang.zhTW',
    en: 'tweets.lang.en',
    ja: 'tweets.lang.ja',
    other: 'tweets.lang.other',
  };
