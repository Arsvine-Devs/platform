...
export default async function ControlPage() {
  ...
  return (
    <main style={{ fontFamily: 'system-ui', margin: '4rem auto', maxWidth: 720, padding: '0 1.5rem' }}>
      <p>ARSVINE CONTROL</p>
      <h1>Control Plane</h1>
      <p>Signed in as {session.email} ({session.role}).</p>
      <section>
        <h2>Authentication</h2>
        <p>Source: {session.authSource === 'oidc' ? 'Auth OIDC' : 'legacy session'}</p>
      </section>
      <section>
        <h2>API</h2>
        {controlPlane ? (
          <p>Connected as {controlPlane.id}; {controlPlane.scopes.length} granted scopes.</p>
        ) : (
          <p role="alert">{controlPlaneError ?? 'Control API validation is not available for this session.'}</p>
        )}
      </section>
    </main>
  );
