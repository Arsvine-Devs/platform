...
beforeEach(() => {
  ...
  vi.mocked(acceptInvitation).mockResolvedValue({
    account,
    invite: { id: '00000000-0000-4000-8000-000000000003' },
  } as never);
  vi.mocked(readActivationToken).mockReturnValue({
  ...
  vi.mocked(decryptSecret).mockReturnValue(
    JSON.stringify({ current: 'JBSWY3DPEHPK3PXP', period: 30, digits: 6, window: 1 }),
  );
...
it('rate-limits invitation activation attempts', async () => {
  ...
  const response = await POST(
    new NextRequest('https://ctrl.arsvine.com/api/auth/invitations/activate', {
      method: 'POST',
  ...
  const response = await POST(
    new NextRequest('https://ctrl.arsvine.com/api/auth/invitations/activate', {
      ...
      headers: {
        ...
        cookie: 'arsvine_invitation_activation=activation-token',
      },
