...
export async function DELETE(
  ...
  if (isDevelopmentBypassSession(session)) {
    ...
    if (!revokeDevelopmentInvitation(id))
      return NextResponse.json(
        { ok: false, error: { message: 'Invitation not found.' } },
        { status: 422 },
      );
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, error: { message: error instanceof Error ? error.message : '无法撤销邀请。' } },
      { status: 422 },
    );
  }
}
