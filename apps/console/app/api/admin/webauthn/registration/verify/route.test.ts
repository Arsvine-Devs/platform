...
beforeEach(() => {
  ...
  vi.mocked(enableOwnerWebAuthn).mockResolvedValue({
    ...owner,
    authMethod: 'webauthn',
    sessionVersion: 2,
  } as never);
...
function request(body: unknown) {
  return new NextRequest('https://ctrl.arsvine.com/api/admin/webauthn/registration/verify', {
    ...
    headers: {
      ...
      cookie: 'arsvine_webauthn_ceremony=00000000-0000-4000-8000-000000000010',
    },
...
describe('POST /api/admin/webauthn/registration/verify', () => {
  it('switches the Owner to WebAuthn and issues a fresh session', async () => {
    ...
    expect(saveWebAuthnCredential).toHaveBeenCalledWith(
      expect.objectContaining({ label: '日常密钥', credentialId: 'credential-id' }),
    );
    expect(enableOwnerWebAuthn).toHaveBeenCalledWith(owner.id, owner.sessionVersion);
    expect(createSession).toHaveBeenCalledWith(
      expect.objectContaining({ authMethod: 'webauthn', sessionVersion: 2 }),
      'webauthn',
    );
