...
if (exportUrl && migrationToken) {
  ...
  if (
    !response.ok ||
    !exported?.releaseId ||
    !Array.isArray(exported.posts) ||
    !Array.isArray(exported.tweets)
  ) {
    throw new Error("Content migration export failed");
  }
  releaseId = exported.releaseId;
  posts = exported.posts;
  tweets = exported.tweets.flatMap((entry) =>
    Array.isArray(entry.tweets) ? entry.tweets : [],
  );
} else {
  const { createObjectStorage, readObjectStorageConfig } =
    await import("../../../packages/object-storage/dist/index.js");
  ...
  const postIndex = manifest.posts?.index
    ? await json(manifest.posts.index)
    : { posts: [] };
  const tweetIndex = manifest.tweets?.index
    ? await json(manifest.tweets.index)
    : [];
  ...
  for (const post of posts) {
    ...
    if (!dryRun) {
      await pool.query(
        ...
        [
          ...
          firstLocale,
          Boolean(post.pinned),
          post.access?.mode ?? "public",
          post.access?.group ?? null,
          publishedAt,
        ],
      );
    ...
    for (const locale of post.availableLocales ?? []) {
      const remoteVariant = Array.isArray(post.variants)
        ...
        : undefined;
      const descriptor = Array.isArray(post.variants)
        ? undefined
      ...
      if (!dryRun) {
        await pool.query(
          ...
          [
            ...
            postId,
            locale,
            variant.title ?? descriptor?.title ?? post.slug,
            variant.excerpt ?? descriptor?.excerpt ?? "",
            variant.bodyMdx ?? "",
            variant.originLocale ?? descriptor?.originLocale ?? null,
          ],
        );
      }
      importedVariants += 1;
    ...
    if (!dryRun) {
      await pool.query(
        ...
        [
          randomUUID(),
          postId,
          JSON.stringify({ source: "content-release", slug: post.slug }),
        ],
      );
    }
  }
