...
export default function BlogWritingPanel({
  form,
  documentKey,
  onChange,
  onEditorError,
}: BlogWritingPanelProps) {
  const { t } = useI18n();
  return (
    <section
      className="flex min-h-0 flex-col overflow-hidden rounded-2xl border bg-card shadow-sm"
      aria-label={`${form.locale} ${t('blog.writing')}`}
    >
      <header className="border-b px-5 py-5 sm:px-7">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 font-heading text-lg font-semibold">{t('blog.writing')}</h2>
          </div>
          <span className="text-xs text-muted-foreground">{t('blog.markdown')}</span>
        </div>
        <FieldGroup className="mt-6">
          <Field>
            <FieldLabel htmlFor="blog-title">{t('blog.heading')}</FieldLabel>
            <Input
              id="blog-title"
              value={form.title}
              onChange={(event) => onChange('title', event.target.value)}
              className="h-11 text-base"
              placeholder={t('blog.headingPlaceholder')}
            />
          </Field>
          <Field>
            <FieldLabel htmlFor="blog-excerpt">{t('blog.excerpt')}</FieldLabel>
            <Textarea
              id="blog-excerpt"
              value={form.excerpt}
            ...
            />
            <FieldDescription>{t('blog.excerptHint')}</FieldDescription>
          </Field>
          <Field>
            <FieldLabel htmlFor="blog-tags">{t('blog.tags')}</FieldLabel>
            <Input
              id="blog-tags"
              value={form.tags}
