...
export default function MembersPageClient({ csrfToken }: { csrfToken: string }) {
  ...
  async function invite() {
    setInviting(true);
    try {
      const result = await adminRequest<InviteData>('/api/admin/members', {
        method: 'POST',
        csrfToken,
        body: { email },
      });
  ...
  async function revokeInvitation(invitation: Invitation) {
    try {
      await adminRequest<void>(`/api/admin/members/invitations/${invitation.id}`, {
        method: 'DELETE',
        csrfToken,
      });
  ...
  async function setStatus() {
    ...
    try {
      await adminRequest<void>(`/api/admin/members/${member.id}`, {
        method: 'PATCH',
        csrfToken,
        body: { status },
      });
      toast.success(
        status === 'disabled' ? t('members.disabledSuccess') : t('members.enabledSuccess'),
      );
      await load();
    } catch (caught) {
      if (isAdminApiError(caught) && (caught.status === 401 || caught.status === 403))
  ...
  const statusDescription =
    pendingStatus?.status === 'disabled'
      ? t('members.disableDescription', { email: pendingStatus.member.email })
      : pendingStatus
        ? t('members.enableDescription', { email: pendingStatus.member.email })
        : '';
  ...
  return (
    <PageFrame size="wide">
        ...
        actions={
          ...
          >
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>
                  {createdLink ? t('members.invitationCreated') : t('members.inviteDialog')}
                </DialogTitle>
                <DialogDescription>
                  {createdLink
                    ? t('members.invitationCreatedDescription', {
                        date: formatDate(createdLink.expiresAt),
                      })
              ...
              {createdLink ? (
                <div className="flex gap-2">
                  <Input
                    ...
                    readOnly
                    aria-label={t('members.invitationLink')}
                  />
                  <Button
                    type="button"
                    variant="outline"
                    ...
                    onClick={() => void copy(createdLink.invitationUrl)}
                  >
                    <Copy />
                  </Button>
                </div>
      ...
      <section className="rounded-2xl border bg-card p-5 shadow-sm sm:p-6">
        <div className="mb-4">
          <h2 className="font-heading text-base font-semibold">{t('members.pendingTitle')}</h2>
          <p className="mt-1 text-sm text-muted-foreground">{t('members.pendingDescription')}</p>
        </div>
        {data.invitations.length === 0 ? (
          <EmptyState title={t('members.noInvites')} className="min-h-28" />
        ) : (
          <>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>{t('members.expires')}</TableHead>
                    <TableHead>
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      {t('members.expires')} {formatDate(invitation.expiresAt)}
                    </p>
                  </div>
                  <Button
                    type="button"
      ...
      <section className="rounded-2xl border bg-card p-5 shadow-sm sm:p-6">
        <div className="mb-4">
          <h2 className="font-heading text-base font-semibold">{t('members.accountsTitle')}</h2>
          <p className="mt-1 text-sm text-muted-foreground">{t('members.accountsDescription')}</p>
        </div>
        <div className="hidden overflow-x-auto sm:block">
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {data.members.map((member) => (
                <MemberRow
                  key={member.id}
                  member={member}
                  onStatusChange={(status) => setPendingStatus({ member, status })}
                />
              ))}
            </TableBody>
          </Table>
        ...
        <div className="grid gap-2 sm:hidden">
          {data.members.map((member) => (
            <article key={member.id} className="rounded-xl border p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium">{member.email}</p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {memberRole(member.role, t)} ·{' '}
                    {t('members.joinedOn', { date: formatDate(member.createdAt) })}
                  </p>
                </div>
                <Badge variant={member.status === 'active' ? 'secondary' : 'outline'}>
                  {memberStatus(member.status, t)}
                </Badge>
              </div>
              {member.role === 'editor' ? (
                <Button
                ...
                <p className="mt-4 text-xs text-muted-foreground">{t('members.ownerProtected')}</p>
              )}
            </article>
          ))}
        </div>
      ...
      <ConfirmAction
        ...
        title={
          pendingStatus?.status === 'disabled'
            ? t('members.disableTitle')
            : t('members.enableTitle')
        }
        description={statusDescription}
        confirmLabel={
          pendingStatus?.status === 'disabled' ? t('members.disable') : t('members.enable')
        }
        destructive={pendingStatus?.status === 'disabled'}
        busy={changingStatus}
...
function MemberRow({
  ...
  return (
    <TableRow>
      ...
      <TableCell>{memberRole(member.role, t)}</TableCell>
      <TableCell>
        <Badge variant={member.status === 'active' ? 'secondary' : 'outline'}>
          {memberStatus(member.status, t)}
        </Badge>
      ...
      <TableCell className="text-right">
        {member.role === 'editor' ? (
          <Button
            ...
            className="min-h-10"
            variant={member.status === 'disabled' ? 'outline' : 'destructive'}
            onClick={() => onStatusChange(member.status === 'disabled' ? 'active' : 'disabled')}
          >
            {member.status === 'disabled' ? (
              t('members.enable')
            ) : (
          ...
          <span className="text-xs text-muted-foreground">{t('members.protected')}</span>
        )}
      </TableCell>
    </TableRow>
  );
